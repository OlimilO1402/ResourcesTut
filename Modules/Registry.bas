...
' #################### ' Local ErrHandler  ' #################### '
Private Sub LocalErrHandler(ByVal hResult As Long, Optional ByVal key As String)
    Dim Buffer As String: Buffer = Space$(256)
    Dim msglen As Long: msglen = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, hResult, 0&, Buffer, Len(Buffer), ByVal 0)
    Buffer = Left$(Buffer, msglen)
    MsgBox "Error: " & hResult & vbCrLf & Buffer & vbCrLf & key
End Sub
